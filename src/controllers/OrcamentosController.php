...
class OrcamentosController extends Controller
{
    ...
    public function processarAcoes()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $veiculo_km = filter_input(INPUT_POST, 'km');
            ...
            if ($acao === 'cadastrar') {
                ...
                try {
                    ...
                    $ultimo_orcamento = Contadores::select()->where('empresa_id', $_SESSION['empresa_id'])->get();
                    $ultimo_orcamento = $ultimo_orcamento[0]['ultimo_orcamento'] + 1;


                    if (empty($cliente_id)) {
                        $cliente_id = Clientes::insert([
                            'nome' => $cliente_nome,
                            'cpf_cnpj' => $cliente_cpf,
                            'telefone' => $cliente_telefone,
                            'data_cadastro' => date('Y-m-d'),
                            'empresa_id' => $_SESSION['empresa_id']
                        ])->execute();

                        $verificarVeiculo = Veiculos::select()
                            ->where('empresa_id', $_SESSION['empresa_id'])
                            ->where('placa', $veiculo_placa)->execute();
                        if (!empty($verificarVeiculo[0])) {
                            Veiculos::update([
                                'cliente_id' => $cliente_id
                            ])->execute();
                            $veiculo_id = $verificarVeiculo[0]['id'];
                        } else {
                ...
                $_SESSION['mensagem'] = "Orçamento excluído!";
                $this->redirect('/orcamentos');
                exit;
                ...
                $this->redirect("/orcamentos/itens?orcamento_id=$orcamento_id");
                ...
                $_SESSION['mensagem'] = "Serviço adicionado!";
                $this->redirect("/orcamentos/itens?orcamento_id=$orcamento_id");
                exit;
                ...
                try {
                    ...
                    $os_id = Ordens_servico::insert([
                        ...
                        'km_veiculo' => $veiculo_km,
                        'data_abertura' => date('Y-m-d'),
                        'status' => 'Aberta',
                        'descricao_problema' => $dados['descricao_servico'],
                    ...
                    Veiculos::update(['km_atual' => $veiculo_km])
                        ->where('empresa_id', $_SESSION['empresa_id'])
                        ->where('id', $dados['veiculo_id'])->execute();
                    ...
                    $endpoint = "/Os/itens?os_id=$os_id";
                } catch (Exception $e) {
                    Model::rollBack();
                    $mensagem = $e->getMessage();
                    $endpoint = "/orcamentos/itens?orcamento_id=$orcamento_id";
                }
